...
test('Green RFP', async ({ page }) => {
  ...
  await page.getByLabel('wednesday June 5').getByText('5').click();
  // await page.pause();
  ...
  await page.locator('md-select[ng-model="vm.question_cut_off_time"]').click();
  // await page.pause();
  await page.locator('div[class="md-text ng-binding"]').isVisible;
  await page.getByRole('option', { name: '12:00 AM' }).locator('div').click();
  await page.locator('md-select[ng-model="vm.submission_due_time"]').isVisible();
  await page.locator('md-select[ng-model="vm.submission_due_time"]').click();
